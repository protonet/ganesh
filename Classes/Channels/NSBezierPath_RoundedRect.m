    ...
    if (!NSIsEmptyRect(rect)) {
        if (radius > 0.0) {
            ...
            if(isActive){
                ...
                // [self lineToPoint:NSMakePoint(NSMaxX(rect)+10, NSMinY(rect))];
                // [self lineToPoint:NSMakePoint(NSMaxX(rect)+10, NSMaxY(rect))];
...
- (void)appendBezierPathWithActiveRoundedRect:(NSRect)rect cornerRadius:(float)radius {
    if (!NSIsEmptyRect(rect)) {
        if (radius > 0.0) {
            // Clamp radius to be no larger than half the rect's width or height.
            float clampedRadius = MIN(radius, 0.5 * MIN(rect.size.width, rect.size.height));
